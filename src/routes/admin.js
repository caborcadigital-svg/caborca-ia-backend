...
async function registrarLog(supabaseAdmin, data) {
  try {
    await supabaseAdmin.from('logs_actividad').insert([data]);
  } catch {}
}
...
router.put('/reportes/:id/estado', async (req, res) => {
  try {
    ...
    if(result.error)throw result.error;
    registrarLog(supabaseAdmin,{accion:'Reporte '+req.body.estado,seccion:'reportes',detalle:'ID: '+req.params.id,usuario_id:req.user.id,usuario_nombre:req.user.nombre||req.user.username});
    res.json(result.data);
...
router.post('/noticias', permisoMiddleware('noticias:crear'), async (req, res) => {
  try {
    ...
    if(result.error)throw result.error;
    registrarLog(supabaseAdmin,{accion:'Noticia publicada',seccion:'noticias',detalle:result.data.titulo,usuario_id:req.user.id,usuario_nombre:req.user.nombre||req.user.username});
...
router.put('/noticias/:id', permisoMiddleware('noticias:editar'), async (req, res) => {
  try {
    ...
    if(result.error)throw result.error;
    registrarLog(supabaseAdmin,{accion:'Noticia editada',seccion:'noticias',detalle:result.data.titulo,usuario_id:req.user.id,usuario_nombre:req.user.nombre||req.user.username});
...
router.delete('/noticias/:id', superadminMiddleware, async (req, res) => {
  try {
    ...
    if(result.error)throw result.error;
    registrarLog(supabaseAdmin,{accion:'Noticia eliminada',seccion:'noticias',detalle:'ID: '+req.params.id,usuario_id:req.user.id,usuario_nombre:req.user.nombre||req.user.username});
...
router.post('/eventos', permisoMiddleware('eventos:crear'), async (req, res) => {
  try {
    ...
    if(result.error)throw result.error;
    registrarLog(supabaseAdmin,{accion:'Evento creado',seccion:'eventos',detalle:result.data.nombre,usuario_id:req.user.id,usuario_nombre:req.user.nombre||req.user.username});
...
router.put('/eventos/:id', permisoMiddleware('eventos:editar'), async (req, res) => {
  try {
    ...
    if(result.error)throw result.error;
    registrarLog(supabaseAdmin,{accion:'Evento editado',seccion:'eventos',detalle:result.data.nombre,usuario_id:req.user.id,usuario_nombre:req.user.nombre||req.user.username});
    res.json(result.data);
...
router.delete('/eventos/:id', superadminMiddleware, async (req, res) => {
  try {
    ...
    if(result.error)throw result.error;
    registrarLog(supabaseAdmin,{accion:'Evento eliminado',seccion:'eventos',detalle:'ID: '+req.params.id,usuario_id:req.user.id,usuario_nombre:req.user.nombre||req.user.username});
    res.json({ok:true});
...
router.post('/deportes/partidos', permisoMiddleware('deportes:crear'), async (req, res) => {
  try {
    ...
    if(result.error)throw result.error;
    registrarLog(supabaseAdmin,{accion:'Partido registrado',seccion:'deportes',detalle:req.body.equipo_local+' vs '+req.body.equipo_visitante,usuario_id:req.user.id,usuario_nombre:req.user.nombre||req.user.username});
    res.status(201).json(result.data);
